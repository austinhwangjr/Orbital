...
{
	f32 t{};
	t += static_cast<f32>(frame_time) * (AEVec2Distance(&player.position, &position) / time_to_player);
...
void Camera::planet_transition(f64 frame_time)
{
	...
	f32 t{};
	t += static_cast<f32>(frame_time) * (AEVec2Distance(&planet_vector[planet_vector.size() - 1].position, &position) / time_to_planet);
